...
trait MiddlewareAwareTrait
{
    ...
    {
        //$callable -> Slim\DeferredCallable
    ...
    {
        //如果添加了中间件　$this->stack　不为 null
        if (is_null($this->stack)) {
            $this->seedMiddlewareStack();
        }

        /** @var callable $start */
        $start = $this->stack->top(); //取出栈的最上面元素

        $this->middlewareLock = true;
        //2次调用执行　依次
        //echo get_class($start); Slim\App , Slim\Route
      
        //直接调用对象　执行魔术方法__invoke
        $resp = $start($req, $res);
